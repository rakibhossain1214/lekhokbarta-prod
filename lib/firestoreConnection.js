...
export async function getAllPostsFrontMatterWithPostId() {
  ...
  const querySnapshotPosts = await getDocs(queryPosts)
  querySnapshotPosts.forEach((doc) => {
    posts.push({ ...doc.data().frontMatter, postId: doc.id, date: doc.data().date, lastmod: doc.data().lastmod })
  })
...
export async function getPostFrontMatterByPostIdAndSlug(postId = '', slug = '') {
  ...
  if (docSnap.exists()) {
    ...
    postData = docSnap.data().frontMatter.slug === slug ? { ...docSnap.data(), postId, date: docSnap.data().date, lastmod: docSnap.data().lastmod } : 'NODATA'
  } else {
